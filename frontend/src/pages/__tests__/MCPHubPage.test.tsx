...
describe('MCPHubPage', () => {
  ...
  it('renders the SkillHub-shaped catalog and selected evidence detail', async () => {
    ...
    expect(screen.queryByText('科研 MCP Hub')).not.toBeInTheDocument()
    ...
    expect(screen.getByRole('heading', { name: '科研 MCP 评测' })).toBeInTheDocument()
  ...
  it('uses the SkillHub-shaped streaming search path', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '查找科研 MCP' }))
    ...
    expect(screen.getByText('已根据当前目录完成推荐。')).toBeInTheDocument()
  })
})
