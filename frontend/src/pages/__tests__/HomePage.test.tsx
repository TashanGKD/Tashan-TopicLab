...
describe('HomePage', () => {
  ...
  it('renders the split hero with grouped home entry controls', () => {
    ...
    expect(screen.getByLabelText('科教生态入口')).toHaveClass('flex', 'flex-col')
    const buttonLabels = screen.getAllByRole('button').map((button) => button.textContent)
    expect(buttonLabels.indexOf('科研应用专区')).toBeLessThan(buttonLabels.indexOf('2026挑战杯专题'))
    expect(buttonLabels.indexOf('2026挑战杯专题')).toBeLessThan(buttonLabels.indexOf('他山青年 TED'))
