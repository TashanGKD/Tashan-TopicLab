...
- 仅将反向代理端口暴露到公网；WorldWeave 容器仍监听 `5000`，宿主机使用 `127.0.0.1:3020` 避开已有服务，并限制来源。
...
- `worldweave`：容器内监听 `5000`，通过 `WORLDWEAVE_HOST_PORT=3020` 绑定宿主机 `127.0.0.1:3020` 的公网缓存优先 Web/API 服务。
...
在独立服务器上设置 `WORLDWEAVE_HOST_PORT=3020`，并将 HTTPS 域名反向代理到 `127.0.0.1:3020`。部署后至少验证：
...
```bash
WORLDWEAVE_BASE_URL=http://host.docker.internal:3020
WORLDWEAVE_UPSTREAM=http://host.docker.internal:3020
VITE_WORLDWEAVE_FRONTEND_URL=/worldweave/
```

如果 WorldWeave 位于另一台服务器，则将前两个值都替换为稳定的 `https://<worldweave-domain>`。
