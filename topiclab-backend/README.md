...
1. 向量包作为版本化 GitHub Release Asset 发布；仓库内 `deploy/topiclink-zvec.lock.json` 固定资产名、SHA-256、文档数和维度。部署会下载到 `${WORKSPACE_PATH}/topiclink-zvec/.downloads`，校验后解压到版本目录。
...
3. GitHub Actions 会把仓库 Secret `DEPLOY_ENV` 写成服务器 `.env`。若其中已有 `SCNET_BASE_URL` 和 `SCNET_API_KEY`，无需新增配置；没有时只补这两个共享接口变量。不要重复配置 Zvec 开关、目录、模型、维度和后台批量参数。发布新向量包时只需上传新的 Release Asset 并更新锁文件。
