...
1. 把预构建压缩包解压到宿主机 `${WORKSPACE_PATH}/topiclink-zvec`。解压后应存在 `${WORKSPACE_PATH}/topiclink-zvec/qwen3-embedding-8b-4096/manifest.*`，不要再多套一层同名目录。
2. 在服务器执行 `chown -R 1000:1000 "${WORKSPACE_PATH}/topiclink-zvec"`，确保镜像内 UID 1000 的 `appuser` 可以增量写入。
3. GitHub Actions 会把仓库 Secret `DEPLOY_ENV` 写成服务器 `.env`。若其中已有 `SCNET_BASE_URL` 和 `SCNET_API_KEY`，无需新增配置；没有时只补这两个共享接口变量。不要重复配置 Zvec 开关、目录、模型、维度和后台批量参数。
